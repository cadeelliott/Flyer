...
class PlaneAnimation(QWidget):
    ...
    def update_position(self):
        try:
            ...
            # Ensure planes are rendered on top of effects
            for plane in self.planes:
                if plane['health'] > 0:
                    plane['label'].raise_()
    ...
    def update_winner_phase(self):
        if self.winner:
            ...
            if self.winner_exit_counter > self.winner_exit_duration and self.winner['pos_x'] > self.screen_width: self.winner['label'].hide(); self.winner = None
                ...
                angle = math.degrees(math.atan2(0, dx)) if dx != 0 else 0
                transform = QTransform().translate(self.plane_size.width()/2, self.plane_size.height()/2).rotate(angle).translate(-self.plane_size.width()/2, -self.plane_size.height()/2)
                self.winner['label'].setPixmap(self.winner['pixmap'].transformed(transform, Qt.SmoothTransformation)); self.winner['label'].move(QPoint(int(self.winner['pos_x']), int(self.winner['pos_y'])))
    ...
    def fire_projectiles(self):
        for i, plane in enumerate(self.planes):
            ...
            if target['health'] > 0:
                ...
                if abs((math.degrees(math.atan2(dy, dx)) - (0 if plane['direction'] == 1 else 180) + 180) % 360 - 180) > 10: continue
                if (dist := math.sqrt(dx**2 + dy**2)) > 1:
                    dx, dy = dx/dist, dy/dist
                    spawn_x, spawn_y = plane['pos_x'] + self.plane_size.width()/2 + (self.plane_size.width()/2 * plane['direction']), plane['pos_y'] + self.plane_size.height()/2
                    proj = {'label': QLabel(self), 'base_pixmap': self.projectile_pixmap, 'pos_x': spawn_x, 'pos_y': spawn_y, 'vx': dx*7, 'vy': dy*7, 'frame': 0, 'shooter_index': i, 'angle': math.degrees(math.atan2(dy, dx))}
    ...
    def fire_missiles(self):
        for i, plane in enumerate(self.planes):
            ...
            if target['health'] > 0:
                dx, dy = target['pos_x'] - plane['pos_x'], target['pos_y'] - plane['pos_y']
                if (dist := math.sqrt(dx**2 + dy**2)) > 400 and abs((math.degrees(math.atan2(dy, dx)) - (0 if plane['direction'] == 1 else 180) + 180) % 360 - 180) < 30:
                    spawn_x, spawn_y = plane['pos_x'] + self.plane_size.width()/2 + (self.plane_size.width()/2 * plane['direction']), plane['pos_y'] + self.plane_size.height()/2
    ...
    def update_planes(self):
        for plane in self.planes:
            ...
            # Calculate speed modifier based on health
            health_percentage = max(0, plane['health'] / 30)
            damage_modifier = 0.4 + (0.6 * health_percentage) # Speed ranges from 40% to 100%
            effective_speed = plane['speed'] * damage_modifier

            plane['prev_pos_x'], plane['prev_pos_y'] = plane['pos_x'], plane['pos_y']
            plane['pos_x'] += effective_speed
            
            if not (-self.plane_size.width() < plane['pos_x'] < self.screen_width):
                plane['pos_x'] = self.screen_width if plane['pos_x'] < -self.plane_size.width() else -self.plane_size.width()
                plane['pos_y'] = random.randint(self.screen_height//4, self.screen_height*3//4); plane['y_target'] = plane['pos_y']
            plane['change_y_counter'] += 1
            if plane['change_y_counter'] >= 120:
                plane['y_target'] = random.randint(self.screen_height//4, self.screen_height*3//4); plane['change_y_counter'] = 0
            ...
            if plane['damage_points']:
                ...
                for point in plane['damage_points']:
                    painter.drawEllipse(point, 4, 4) # 4px radius hole
                painter.end()
            ...
            for i in range(num):
                y_offset = (i - (num - 1) / 2) * 10
                sx, sy = plane['pos_x'] + self.plane_size.width()/2 - (self.plane_size.width()/10 * plane['direction']), plane['pos_y'] + self.plane_size.height()/2 + y_offset
                smoke = {'label': QLabel(self), 'pos_x': sx, 'pos_y': sy, 'vx': 0.1*plane['direction'], 'vy': 0, 'size': size, 'opacity': 0.8, 'lifetime': 60, 'frame': 0, 'type': 'smoke'}
            ...
            if self.game_active:
                for j, plane in enumerate(self.planes):
                    if plane['health'] > 0 and j != p['shooter_index'] and p['label'].geometry().intersects(plane['label'].geometry()):
                        ...
                        # Add a cluster of 3 holes for more visible damage
                        impact_x = random.randint(20, self.plane_size.width() - 20)
                        impact_y = random.randint(20, self.plane_size.height() - 20)
                        for _ in range(3):
                            hole_x = impact_x + random.randint(-5, 5)
                            hole_y = impact_y + random.randint(-5, 5)
                            plane['damage_points'].append(QPoint(hole_x, hole_y))
